...
: 다시 사용할 수 있음(효율적)\
: 코드를 관리하기 쉬워짐\
: 조립해서 사용할 수 있음(작은단위의 함수들을 조합해 금방금방 프로그램 만들기 가능)

(2) Docstring(문서화 문자열)
: 함수가 어떤 일을 수행하는지 설명하는 주석 중 하나이다.   
- 형태

       """여기에 달고싶은 주석을 쓰면 된당 키키"""
- 예제
...
: Phython에 포함되어 있기 때문에 우리가 별도의 작업을 할 필요없이 호출해서 사용 가능한 함수.
(1) input()
...
#### 2. 모듈의 함수
: 다른 곳에서 이미 만들어 놓은 모듈에 포함되어있는 함수

#### 3.사용자 정의 함수
: 직접 만들어서 내가 원하는 대로 사용가능 한 함수
- 형태
       ...
       def 함수이름(인자1,...):
           ...
           return 결과
>이때 결과를 결과1,결과2,... 등으로 나타낼 수 있는데 그것들은 패킹(packing)이되어 모두 튜플로 저장된다.
- 예제

       def my_sum(n1,n2):
           return n1+n2, n1*n2
       result = my_sum(2,3)
       print(result) 하면 (5,6) 이라고 출력된다.

### 모듈
: 비슷한 함수들을 모아둔 파일.\
: import라는 키워드를 사용해 가지고 올 수 있다.\
: .을 이용해 모듈에 포함되어 있는 함수를 사용할 수 있다.\
: Phython에는 미리 준비되어 있는 모듈이 많아서 이러한 모듈들을 가져와서 쓰면 된다.
#### 1. random 모듈(대표적)
: 난수를 만들거나 무작위와 관련된 함수를 포함하고 있는 모듈

(1) .choice()\
: random 모듈에 포함되어있는 함수로, list 입력받았을 때 안에 있는 값중에서 하나를 랜덤하게 뽑아주는 함수.
- 예제
...
(2) .sample()\
: random 모듈에 포함되어있는 함수로, list 입력받았을 때 안에 있는 값중에서 정해진 숫자만큼 중복없이 랜덤하게 뽑아주는 함수.
- 형태
       
       random.sample(리스트, 몇개의 값을 중복없이 선택할건지)
 >리스트형태로 뽑아진다.      
- 예제
...
(3) .randint()\
: random 모듈에 포함되어있는 함수로, 정해진 정수 범위 내에서 랜덤하게 정수 하나를 선택해서 돌려주는 함수.
- 형태
...
### 더 알아보기
#### 객체
: 현실에 사물, 물체, 물건 등을 컴퓨터안에 재현한 것.
- 함수와 데이터를 한꺼번에 묶어서 표현하게 된다.
- 객체가 가지고 있는 데이터와 객체가 취할 수 있는 행동! 이 두가지로 표현가능
