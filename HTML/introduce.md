...
#### ```<select>``` 태그 / ```<option>``` 태그
- ```<select>``` 태그
: 미리 준비된 여러 개의 선택지를 골라 선택할 수 있는 선택 박스(Select box)\
: ```<select>```태그는 ```<option>``` 태그로 이루어져 있는 각 선택지를 하나로 감싸는 역할을 수행.\
: name이라는 속성을 무조건 가져야한다.
``` html
...
```
- ```<option>``` 태그
: value라는 속성을 무조건 가져야한다.\
: ```<select>```의 name ```<option>```의 value가 짝을 이룸.
> ```<input>```에서는 한 태그 안에 있는 name과 value가 짝.

#### ```<textarea>``` 태그
: 한 번에 많은 양의 글을 입력 받을 때 사용.\
: rows, cols 속성과 함께 사용 가능
> rows : 글 줄 수, cols : 글자 수
``` html
...
#### ```<button>``` 태그
: HTML 요소를 ```<button>``` 태그 내부에 담아 사용할 수 있다는 장점이 있음.
> 이미지 버튼 제작도 가능.
``` html
...
### 목록(List)
> 일반적인 텍스트와는 다르게 들여쓰기가되어 리스트 범위에 포함되는 관계임을 나타냄.
>> 브라우저의 디폴트 스타일이 적용되고 있음.
#### 1. 순서 없는 목록(Unordered List)
- ```<ul>```
: 순서 없는 리스트의 항목을 감싸는 태그
- ```<li>```
...
> 지정안하면 앞의 숫자와 연속인 숫자가 나옴.
>> 그냥 순서 있는 목록 태그 속성 start랑 비슷하다고 생각하면 될 듯.

#### 2. 순서 있는 목록(Ordered List)
- ```<ol>```
: 순서 있는 리스트의 항목을 감싸는 태그
- ```<li>```
: 리스트에 나열할 항목을 담아두는 태그
``` html
<ol>
